...
import { subscribePublicRepair } from '../lib/repairs'
...
function categoryLabel(repair) {
  ...
  if (repair.itemName) return `${cat}: ${repair.itemName}`
  return cat
}
...
export default function TrackStatus() {
  ...
  useEffect(() => subscribePublicRepair(publicId, setRepair), [publicId])
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-6 space-y-5">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-orange-100 p-5 space-y-3">
        ...
        {repair.brand && <p className="text-sm text-slate-500">ยี่ห้อ: {repair.brand}</p>}
        <PhotoOrPending
