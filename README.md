...
7. **ปิดงาน/ส่งมอบคืน** (`/repairs/:id/close`) — ถ่ายรูปสิ่งของและผู้รับคืน บันทึกชื่อผู้รับคืน
   แล้วปิดงานเป็นสถานะ "ส่งมอบ/ส่งคืนแล้ว" (10) — เป็นทางเดียวที่จะไปสถานะนี้ได้ ยกเลิกได้ด้วยปุ่ม
   "ยกเลิกปิดงาน/ส่งมอบ" ในหน้ารายละเอียด (แสดงแทนปุ่ม "ปิดงาน/ส่งมอบ" เดิมเมื่อสถานะเป็น 10) ซึ่ง
   ย้อนกลับไปสถานะ 8/9 (ตามธง `unrepairable`) และล้างข้อมูล `closure` ทิ้ง (`reopenRepair()`)
...
2. คัดแยก/ประเมิน
3. ชะล้างดินโคลน
4.1. รอคิวซ่อม ณ ศูนย์ซ่อมรถจักรยานยนต์
4.2. กำลังซ่อม ณ ศูนย์ซ่อมรถจักรยานยนต์
5.1. รอคิวซ่อม ณ ศูนย์ซ่อมรถยนต์
5.2. กำลังซ่อม ณ ศูนย์ซ่อมรถยนต์
6.1. รอคิวซ่อม ณ สถานีซ่อมและบำรุงรักษาเครื่องใช้ไฟฟ้า
6.2. กำลังซ่อม ณ สถานีซ่อมและบำรุงรักษาเครื่องใช้ไฟฟ้า
7. ตรวจสอบคุณภาพ
8. ซ่อมเสร็จแล้ว/รอส่งมอบ
9. ไม่สามารถซ่อมได้/รอส่งคืน
10. ส่งมอบ/ส่งคืนแล้ว

สถานะ 4/5/6 (แต่ละสถานีซ่อม) ใช้เลขทศนิยม `.1`/`.2` แยก "รอคิว" กับ "กำลังซ่อมจริง" — ตั้งใจ
ไม่เลื่อนเลขสถานะ 7 ขึ้นไปเมื่อเพิ่มสถานะย่อยเหล่านี้ เพราะเลขสถานะถูกบันทึกไว้ในข้อมูลจริงแล้ว
(`repairs/{id}.status`, `stats/summary.byStatus`) เลื่อนเลขของสถานะที่มีอยู่แล้วจะทำให้ข้อมูล/
สถิติสะสมของรายการเก่าคลาดเคลื่อน — รายการที่เคยค้างอยู่ที่สถานะ 4/5/6 แบบเดิม (ไม่มีทศนิยม)
ก่อน deploy ครั้งนี้ ยังแสดงป้ายชื่อถูกต้อง (ผ่าน `LEGACY_STATUS_LABELS` ใน `src/lib/options.js`)
และกดอัปเดตสถานะต่อได้ตามปกติ เพียงแต่ไม่ปรากฏเป็นตัวเลือกในดรอปดาวน์อีกต่อไป
